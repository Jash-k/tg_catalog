...
def parse_filename(raw: str, channel: dict) -> ParsedName:
    ...
    text = re.sub('|'.join(WATERMARKS), ' ', raw)
    ...
    # Multi-audio release blocks such as [Telugu + Tamil + Hindi + English]
    # are treated as Tamil-audio/dubbed when Tamil appears with another language.
    if re.search(r'(?i)\btamil\b', raw) and re.search(r'(?i)\b(?:telugu|hindi|malayalam|kannada|english|japanese)\b', raw):
        dubbed = True
    ...
    if title_year_match: title = title[:title_year_match.start()]
