...
class TMDB:
    ...
    async def details(self, tmdb_id, media_type):
        d = await self.get(f'/{"tv" if media_type == "series" else "movie"}/{tmdb_id}', {'append_to_response':'credits,translations'})
        ...
        return {'tmdb_id': d['id'], 'media_type': media_type, 'title': title, 'english_title': title,
